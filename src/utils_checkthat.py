        ...
        for sample in data:
            ...
            if len(relations) > 0:
                ...
                for tgt_lang in self.tgt_langs:
                    ...
                    sample_dict.update({
                        f'{self.text_field}_{tgt_lang}': sample['data'][f'{self.text_field}_{tgt_lang}'],
                        f'ents_{tgt_lang}': ents_tgt,
                    })
                sample_dict.update({
                    f'{self.text_field}_{self.src_lang}': sample['data'][f'{self.text_field}_{self.src_lang}'],
...
class EntityShifter:
    def __init__(self, languages = ['en', 'it'], mappings = regex_tokenizer_mappings) -> None:
        ...
        if strategy == 'moses':
            ...
            for lang in self.languages:
                ...
                if data_format == 'label_studio':
                    ...
                    text = sample[f'{text_field}_{lang}']
                    ents = sorted(sample[f'ents_{lang}'], key = lambda ent: ent[0])
                ...
                if data_format == 'label_studio':
                    ...
                    sample[f'{text_field}_{lang}'] = text
                annotations += ents
                if verbose:
                    text_reconstructed = ' '.join([text[ent['value']['start']:ent['value']['end']] for ent in ents if ent['from_name'] == f'label_{lang}'])
                    ic(text)
